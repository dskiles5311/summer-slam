        ...
        {unassigned.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{
              ...
              border: '1px solid rgba(139,180,225,0.25)',
              borderRadius: 8,
              padding: '10px 16px',
              marginBottom: 10,
              display: 'flex',
              ...
              <div>
                <div style={{ fontWeight: 700, color: 'var(--header-bg)', fontSize: 15 }}>Unassigned</div>
                <div style={{ fontSize: 12, color: 'rgba(168,200,160,0.6)', marginTop: 2 }}>
                  {unassigned.length} boat{unassigned.length !== 1 ? 's' : ''} — not in any configured flight range
                </div>
              </div>
            </div>
            {unassigned.map(entry => <BoatRow key={entry.id} entry={entry} />)}
          </div>
        )}
      </div>
    </div>
