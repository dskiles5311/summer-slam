              ...
              <div onClick={() => openCard(row)} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--header-bg)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {row.coAnglerFirst || row.coAnglerLast ? `${row.coAnglerFirst} ${row.coAnglerLast}`.trim() : 'No co-angler'}
                  </div>
                </div>
              ...
              {isOpen && (
                <div style={{ padding: '10px 12px 12px', borderTop: '1px solid rgba(168,200,160,0.12)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, alignItems: 'end' }}>
                    ...
                    <div>
                      <span style={LABEL}>Lunker</span>
                      <button type="button" onClick={() => toggle('lunker')} style={{
                        width: '100%', padding: '6px 0', borderRadius: 6, fontWeight: 700, fontSize: 13, cursor: 'pointer',
                        border: `1.5px solid ${isOn(draft.lunker) ? '#4CAF50' : 'rgba(168,200,160,0.3)'}`,
                        background: isOn(draft.lunker) ? 'rgba(76,175,80,0.18)' : 'rgba(255,255,255,0.04)',
                        color: isOn(draft.lunker) ? '#4CAF50' : 'var(--header-bg)',
                      }}>
                        {isOn(draft.lunker) ? 'YES' : 'NO'}
                      </button>
                    </div>
                    <div>
                      <span style={LABEL}>Option</span>
                      <button type="button" onClick={() => toggle('option')} style={{
                        width: '100%', padding: '6px 0', borderRadius: 6, fontWeight: 700, fontSize: 13, cursor: 'pointer',
                        border: `1.5px solid ${isOn(draft.option) ? '#4CAF50' : 'rgba(168,200,160,0.3)'}`,
                        background: isOn(draft.option) ? 'rgba(76,175,80,0.18)' : 'rgba(255,255,255,0.04)',
                        color: isOn(draft.option) ? '#4CAF50' : 'var(--header-bg)',
                      }}>
                        {isOn(draft.option) ? 'YES' : 'NO'}
                      </button>
                    </div>
                  </div>
